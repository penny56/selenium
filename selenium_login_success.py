...
def login_test():
    ...
    try:
        ...
        print("开始登录流程...")
        login_page.login(login_page.username, login_page.password)

        WebDriverWait(driver, 10).until(
            EC.url_contains('practicetestautomation.com/logged-in-successfully/')
        )
        print('页面跳转成功，URL 校验通过')


        WebDriverWait(driver, 10).until(
            EC.text_to_be_present_in_element((By.TAG_NAME, 'body'), 'successfully logged in')
        )
        print('页面包含预期文本！')


        # 等待 Log out 按钮出现在页面上并可见
        WebDriverWait(driver, 10).until(
            EC.visibility_of_element_located((By.XPATH, "//a[text()='Log out']"))
        )
        print("Log out 按钮已成功显示")
